...
Item {
  ...
  WlSessionLock {
    ...
    WlSessionLockSurface {
      ...
      LockView {
        ...
        coreTemperature: root.coreTemperature
  ...
  Timer {
    interval: 5000
    repeat: true
    running: root.lockRequested || root.previewVisible
  ...
  // Prefer a coretemp sensor and gracefully fall back to a thermal zone.
  Process {
    id: telemetryProc
    command: ["bash", "-c", "temp=''; for hw in /sys/class/hwmon/hwmon*; do [ \"$(cat \"$hw/name\" 2>/dev/null)\" = coretemp ] || continue; for sensor in \"$hw\"/temp*_input; do value=$(cat \"$sensor\" 2>/dev/null); [ \"$value\" -gt 0 ] 2>/dev/null && { [ -z \"$temp\" ] || [ \"$value\" -gt \"$temp\" ]; } && temp=$value; done; done; if [ -z \"$temp\" ]; then for sensor in /sys/class/thermal/thermal_zone*/temp; do value=$(cat \"$sensor\" 2>/dev/null); [ \"$value\" -gt 0 ] 2>/dev/null && { [ -z \"$temp\" ] || [ \"$value\" -gt \"$temp\" ]; } && temp=$value; done; fi; mem=$(awk '/MemTotal:/ { total=$2 } /MemAvailable:/ { available=$2 } END { if (total > 0) printf \"%.0f\", (total-available)*100/total; else print \"--\" }' /proc/meminfo); up=$(awk '{ printf \"%02d:%02d\", int($1/3600), int($1/60)%60 }' /proc/uptime); load=$(awk '{ printf \"%.2f\", $1 }' /proc/loadavg); [ -n \"$temp\" ] && temp=$((temp / 1000)); printf '%s|%s|%s|%s\\n' \"${temp:---}\" \"$mem\" \"$up\" \"$load\""]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        var fields = String(text || "").trim().split("|")
        if (fields.length !== 4) return
        root.coreTemperature = fields[0] === "--" ? "--°C" : fields[0] + "°C"
        root.memoryUsage = fields[1] + "%"
        root.systemUptime = fields[2]
        root.loadAverage = fields[3]
      }
    }
  }
